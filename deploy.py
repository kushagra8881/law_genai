...
# Initialize ChromaDB connection using ChromadbConnection
configuration = {"client": "PersistentClient", "path": "/tmp/.chroma"}
collection_name = "indian_law_bge_work_1"
...
def vector_search(query, top_k=5):
    try:
        results = conn.query(query_texts=[query], collection_name=collection_name, n_results=top_k)
...
st.info("This is a law assistant that can answer questions about Indian law using vector search and a language model.")
st.divider()
st.image("image.png")
...
if st.button("Submit"):
    ...
    formatted_prompt = prompt.format(query=query, fused_results=fused_results_str)
    ...
    for result, score in fused_results:
